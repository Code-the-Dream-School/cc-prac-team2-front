...
const ChatInput = ({
  ...
  const handleOpenEmojiPicker = (event: React.MouseEvent<HTMLDivElement>) => {
    const { top, left } = event.currentTarget.getBoundingClientRect();
    setEmojiPickerPosition({ top: top + 20, left });
  };
          ...
          <form onSubmit={handleSendMessage} className="flex flex-row">
            <div className="m-auto pl-6" onClick={handleShowEmoji}>
              {!showEmoji ? (
                ...
                <div
                  className="absolute left-100 top-28 h-12 w-12"
                  style={emojiPickerPosition}
                >
                  <span
                    className="relative z-50 top-5 left-80 w-32 items-center justify-start cursor-pointer "
